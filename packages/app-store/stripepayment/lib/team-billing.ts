...
export function getRequestedSlugError(error: unknown, requestedSlug: string) {
  ...
  if (error instanceof PrismaClientKnownRequestError && error.code === "P2002") {
    statusCode = 400;
    message = `It seems like the requestedSlug: '${requestedSlug}' is already taken. Please contact support at ${process.env.NEXT_PUBLIC_SUPPORT_MAIL_ADDRESS} so we can resolve this issue.`;
